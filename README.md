...
В коде в некоторых местах оставил пометки по тому или иному вопросу. Учитывая кодстайл, делал их на английском.
Где-то у методов решил делать epression body, где-то statement в зависимости от того понадобится ли дебажить метод.

**Подход:** думал взять сначала ECS, т.к. статы персонажа/модификаторы хорошо ложаться на Entity с подобным геймплеем.
...
**Архитектура:** подходил к ней как не к ТЗ (пускай оно и просто и его можно на MonoBehaviour всё сделать), а к реальному проекту, который нужно будет развивать
- Точка входа: стартовая сцена InitialScene BootstrapInstaller - биндятся основные сервисы здесь. В качестве DI-Container-а выбран Zenject.
- Стейты-приложения: игра разбита на стейты, позволяя удобно их отделять. BootstrapState прогревает статик дату, InitializeNaniState - иницианизирует нани (делаю это руками т.к. иногда отключаю его для тестов + дальше по пайплайну нужно инжектить его UI)
...
- Все лайфайм сущности билдятся рантайм через фабрику GameFactory (кроме карты деревни, т.к. на ней составляются локации). Фабрика пока что не такая больашя, но в будущем бы разделил на несколько других фабрик, если бы выросла дальше.
...
Подумал как это будет просто для геймдиза (в рамках ТЗ конечно) - просто расставить LocationPoints и нажать Collect. Если гейм диз совсем не знаком с юнити можно ещё более удобную и даже автоматическую систему
в реальном проекте.
...
**Второй день:** более полноценная сервисная логика по задачам из ТЗ, добавление ассет-графики, UI.
