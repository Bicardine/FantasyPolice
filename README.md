...
В коде в некоторых местах оставил пометки по тому или иному вопросу. Учитывая кодстайл, делал их на английском. Это всё обсуждаемо и заменяемо, в зависимости от команды c:

Где-то у методов решил делать expression body, где-то statement в зависимости от того понадобится ли дебажить метод
...
- Точка входа: стартовая сцена InitialScene (запускать с неё) и BootstrapInstaller - биндятся основные сервисы здесь. В качестве DI-Container-а выбран Zenject в демонстрационных целях. На данном этапе он не прямо здесь нужен, но упрощает резолв зависимостей.
- В дальнейшем бы точно какой-то контейнер скорее всего понадобился, хотя можно и руками резолвить, если игра не будет особо обширной.
- Стейты-приложения: игра разбита на стейты, позволяя удобно их отделять. BootstrapState прогревает статик дату и прочие будущие сервисы, InitializeNaniState - иницианизирует нани (делаю это отдельно т.к. иногда отключаю его для тестов + дальше по пайплайну нужно инжектить его UI)
- RegisterNaniState: учитывая не самую удобную фабрику нани (без возможности встроиться в неё override-ом или подменить), бинд зависимостей по наниновел части происходит здесь. Думал вообще вставлять ли Нани, но решил использовать его хотя бы для UI,
  раз основной рабочий проект использует этот фрейм.
...
- Все лайфайм сущности билдятся рантайм через фабрику GameFactory (кроме тайлмапы деревни, т.к. на ней составляются локации). Фабрика пока что не такая большая, но в будущем бы разделил на несколько других фабрик, если бы выросла дальше.
- Обращение к статик дате (prefabs assets/SO) - происходит через сервис StaticData. Там реализация ассет провайдера на Resources, у меня есть аналогичная на Addressables но в рамках ТЗ решил не подключать её.
  + пока нет смысла работать с загрузкой/выгрузкой ассетов, не увидел предпосылки такого рода игры к подобному.
- Service-ориентированная модель, где логика и обращения проистекает через свои небольшие сервисы.


**По части кода**: где-то сразу сделал абстракции, зная, что в таком типе игр скорее всего это пришлось бы сделать в будущем, где-то остановился на более явных реализациях в рамках ТЗ и сроков.
...
Подумал как это будет попроще для геймдиза (в рамках ТЗ и времени, конечно) - просто расставить LocationPoints и нажать Collect, они автоматически подхватятся. Если гейм диз совсем не знаком с юнити можно подумать над ещё более упрощённой версие.

<img width="1920" height="515" alt="Points" src="https://github.com/user-attachments/assets/a90b8e64-98f8-44a2-8b91-fe03ddc93b44" />


- Систему модификаторов статов (можно выбрать Mode: Stack/Instance), если Stack - то модификатор одного и того же типа будет стакаться (по-прежнему увеличивая Value-баффа/дебаффа) и отображаться как один эффект на карте,
...
Конфиг позволяет указывать различные модификаторы к статам
...
- LogVersion - база любого рабочего проекта для удобства дебагинга и просмотр версии, т.к. иногда тестеры сами не уверены какую версию тестят c:
...
С какими-либо трудностями не столкнулся, довольно привычные задачи. В целом было залипательно делать такие механики, нравятся карточные игры, понравилось c: Без проблем вольюсь в другой тип архитектуры и подход в программировании.
